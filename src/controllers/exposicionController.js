...
export const getAllExposiciones = async (req, res) => {
  try {
    ...
    if (!Array.isArray(exposiciones)) {
      console.error('findAll NO devolvió un array:', exposiciones);
      return res.json([]);
    }

    const resultado = exposiciones.map(e => {
      let integrantes = [];
      let archivos_programas = [];
      let db_files = [];

      try {
        integrantes =
          typeof e.integrantes === 'string'
            ? JSON.parse(e.integrantes || '[]')
            : (Array.isArray(e.integrantes) ? e.integrantes : []);
      } catch {
        integrantes = [];
      }
      ...
      try {
        archivos_programas =
          typeof e.archivos_programas === 'string'
            ? JSON.parse(e.archivos_programas || '[]')
            : (Array.isArray(e.archivos_programas) ? e.archivos_programas : []);
      } catch {
        archivos_programas = [];
      }
      ...
      try {
        db_files =
          typeof e.db_files === 'string'
            ? JSON.parse(e.db_files || '[]')
            : (Array.isArray(e.db_files) ? e.db_files : []);
      } catch {
        db_files = [];
      }
...
export const getExposicion = async (req, res) => {
  try {
    const { id } = req.params;
    const e = await Exposicion.findById(id);
    if (!e) return res.status(404).json({ message: 'Exposición no encontrada' });
